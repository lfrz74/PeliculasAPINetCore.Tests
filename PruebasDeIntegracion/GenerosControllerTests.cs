...
namespace PeliculasAPI.Tests.PruebasDeIntegracion
{
    ...
    public class GenerosControllerTests : BasePruebas
    {
        ...
        public async Task ObtenerTodosLosGenerosListadoVacio()
        {
            ...
            var generos =
                JsonConvert.DeserializeObject<List<GeneroDTO>>(await respuesta.Content.ReadAsStringAsync());
        ...
        {
            var nombreBD = Guid.NewGuid().ToString();
            var factory = ConstruirWebApplicationFactory(nombreBD); //, false); //Con false falla 401 Unauthorized v
